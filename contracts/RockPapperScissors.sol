...
contract Game {
    ...
    event PlayerSelections(uint256 gameNumber, string player, address account, Options selection, string selection_string);
    ...
    function startGame() internal returns (string memory) {
        require(allGames[_gameCount].playerA != address(0x0), "The first player has not selected their play choice yet.");
        require(allGames[_gameCount].playerB != address(0x0), "The second player has not selected their play choice yet.");
    ...
    function playWithComputer(Options selected) public returns (string memory) {
        require(allGames[_gameCount].concluded == true, "Please finish the existing game before starting a game with the computer.");
        ...
        emit PlayerSelections(_gameCount, "PlayerA", msg.sender, selected, mapSelectionToString(selected));
        emit PlayerSelections(_gameCount, "PlayerB: Computer", address(this), computerSelected, mapSelectionToString(selected));

        string memory result = startGame();

        return string.concat("PlayerA selected: ", mapSelectionToString(selected), ". ", 
        "PlayerB (Computer) selected: ", mapSelectionToString(computerSelected), ". ", result);
    }

    function selectOption(Options selected) public returns (string memory){
        // require(selected == Options, "Invalid option selected");
        if(allGames[_gameCount].playerA == address(0x0)){
            ...
            require(allGames[_gameCount].playerA != msg.sender, "PlayerB cannot be the same player as playerA, please select option with a different account.");
            allGames[_gameCount].playerB = msg.sender;
            allGames[_gameCount].playerBSelection = selected;
            emit PlayerSelections(_gameCount, "PlayerB", msg.sender, selected, mapSelectionToString(selected));
            string memory result = startGame();
            return string.concat("PlayerB selected: ", mapSelectionToString(selected), ". ", result);
        }
    }

    function mapSelectionToString(Options selected) internal pure returns (string memory){
    ...
    function mapSelectionToEnum(uint256 selected) private pure returns (Options){
        if (selected == 0) {
            return Options.ROCK;
        } else if (selected == 1) {
    ...
    function determineWinner(GameInfo memory game) internal returns (string memory result) {
        if (game.playerASelection == game.playerBSelection) {
            allGames[_gameCount].Winner = game.playerA;
            return string.concat("Both players selected: ", mapSelectionToString(game.playerASelection), "It's a tie!");
